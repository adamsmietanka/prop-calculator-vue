<template>
  <div>
    ...
    <NumberInput
      name="Angle"
      v-if="bladePitch === 'Fixed'"
      :number="prop.angle"
      rules="required|between:10,60"
      :setter="setAngle"
      unit="°"
    />
    <DisabledInput name="Power" :model="prop.cruisePower" unit="kW" />
    <DisabledInput name="Cn" :model="Cn" />
    <b-form-group label="Blade material">
      <b-form-radio-group
        v-model="bladeMaterial"
        :options="['Metal', 'Composite', 'Wood']"
      />
    </b-form-group>
    <DisabledInput name="Tip Mach Number" :model="tipMach" :rules="rulesTip" />
    <b-button block variant="primary" @click="update" class="mt-2"
      >Optimize Diameter</b-button
    >
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    numberOfBlades: {
      get() {
        return this.prop.numberOfBlades;
      },
      set(v) {
    ...
    bladeMaterial: {
      get() {
        return this.prop.bladeMaterial;
      },
      set(v) {
        this.$store.dispatch("setBladeMaterial", v);
      },
    },
    bladePitch: {
      get() {
        return this.prop.bladePitch;
      },
      set(v) {
        this.$store.dispatch("setBladePitch", v);
      },
    },
    Cn() {
      ...
      const Cn =
        maxSpeed *
        (this.rho / (cruisePower * 1000 * this.propSpeed ** 2)) ** 0.2;
