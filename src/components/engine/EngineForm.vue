<template>
  <div class="engine-form">
    ...
    <b-form-group label="Supercharger" v-show="engineType === 'Piston'">
      <SuperchargerForm
        v-for="(stage, index) in this.engine.supercharger"
        :key="stage.id"
        :index="index"
      />
      <b-button :disabled="stages === 2" variant="primary" @click="addStage">
...
export default {
  ...
  computed: {
    ...
    maxAltitude: {
      get() {
        return this.engine.maxAltitude;
      },
      set(v) {
        this.$store.dispatch('setMaxAlt', v);
      },
    },
    kCoefficient: {
      get() {
    ...
    engineType: {
      ...
      set(v) {
        this.$store.dispatch('setEngineType', v);
      },
    },
    turbo: {
      get() {
        return this.engine.turbocharger.enabled;
      },
      set(v) {
        this.$store.dispatch('toggleTurbo', v);
      },
    },
  },
  methods: {
